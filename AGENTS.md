...
- **Binary paths (Prime Agent / uv / Python)**: Preferences → System and the desktop launcher share the same editor (`public/components-settings.js`, `/api/system/components*`). Storage is `engine/selection.json` + `installation.json` under `componentsDataRoot()` (`lib/desktop-components.mjs`). Env vars win; external picks validate by capability (not exact policy pin); managed install still pins `COMPONENT_POLICY`. Loopback-only — not on the LAN allowlist.
