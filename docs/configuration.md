...
**Outils** donne accès aux MCP et aux catalogues Skills/Prompts, avec leurs dossiers globaux et du projet sélectionné. **Accès distant**, réservé au PC, active le LAN et Tailscale sans interrompre les agents et propose les liens et QR codes : voir [le guide mobile](lan.md). **Système** affiche les versions, la disponibilité du moteur et le nombre d’agents en cours ; le diagnostic copiable exclut les clés et conversations. L’ouverture des journaux est réservée au PC. Sur le Studio local (navigateur ou application Linux), **Système** permet aussi de régler les chemins de **Prime Agent**, **uv** et **Python**, avec un indicateur vert/rouge après validation. **Détecter les installations** propose les outils trouvés sur le PATH ; **Installer les manquants** télécharge encore les versions gérées de la politique. Les variables d’environnement l’emportent sur les chemins enregistrés. Ces routes sont réservées au loopback et absentes de la passerelle LAN.
