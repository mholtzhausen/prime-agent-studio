...
Le chargeur local `runtime/headless-loader.mjs` active l’attente native de fin des sous-agents avant que le client JSON ferme sa session. La réponse du parent ne coupe donc pas les tâches qu’il vient de déléguer. Les exécutions interactives utilisent `runtime/studio-rpc-loader.mjs` pour exposer `studio_wait_for_completion` sur la boucle RPC. À partir de prime-agent 0.9.5, l’entrée npm `dist/bundle/cli.js` est un pont qui réexécute `cli-node.js` avec les mêmes arguments Node `--import` / `--require` ; les deux chargeurs conservent `PRIME_GUI_CLI_ROOT` jusqu’à cette entrée réelle pour que le correctif en mémoire ne soit pas perdu sur le processus pont. Les changements s’appliquent uniquement au mode d’exécution du Studio ; les fichiers installés de Prime Agent restent intacts. Si une mise à jour du CLI change un point d’intégration, le Studio affiche une erreur explicite plutôt que d’appliquer une transformation incertaine.
