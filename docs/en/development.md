...
Studio does not provision Python, run `uv`, manage `.local/kernel-venv/`, or inject `runtime/kernel-loader.mjs` for kernel prep. **Prime Agent** bootstraps Python skills itself. Advanced hosts may set `PRIME_AGENT_KERNEL_PYTHON` as a Prime Agent environment override; Studio does not install into it. See the [native documentation](https://github.com/PrimeIntellect-ai/prime-agent/blob/main/packages/coding-agent/docs/skills.md#python-backed-skills).

`lib/kernel.mjs` still exposes `localKernelPython` and `execute` helpers for MCP probe discovery and process execution when a marker or PATH interpreter is available. `npm run setup:runtime` / `make setup-runtime` is a leftover no-op. `test/kernel.test.mjs` covers legacy `ensureLocalKernel` helpers if still present. `test:subagents:native` also includes `scripts/test-kernel-messaging-native.mjs`: a simulated localhost provider, real kernels owned by Prime Agent, explicit messages in both directions verified in histories **and** model contexts, then resuming the same parent after engine shutdown. A child-completion notification does not satisfy this test.
...
The local `runtime/headless-loader.mjs` loader enables native waiting for subagent completion before the JSON client closes its session. A parent response therefore does not stop newly delegated tasks. Interactive runs use `runtime/studio-rpc-loader.mjs` to expose `studio_wait_for_completion` on the RPC loop. From prime-agent 0.9.5 the npm entry `dist/bundle/cli.js` may hand off to a compiled native binary (dropping Node `--import` / `--require`). Interactive and headless Studio runs therefore spawn `dist/bundle/cli-node.js` via `resolvePrimeAgentNodeEntry` so the in-memory patches stay applied. If a Node→Node bridge re-exec still occurs, both loaders keep `PRIME_GUI_CLI_ROOT` until that real entry. Changes apply only to Studio’s execution mode; installed Prime Agent files stay intact. If a CLI update changes an integration point, Studio reports an explicit error instead of applying an uncertain transformation.
...
Build the native application and packages with `make build` (unsigned) or `make build-release` (signed + updater catalog): see [the Linux application guide](desktop.md). Native tests use a temporary folder and dedicated port. The interface has no generic Tauri shell access; launcher commands check their local origin, and external links open in the browser.
