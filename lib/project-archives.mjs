 ...
 * pending-journal recovery, model-selection gate. Delegates:
 ...
 * - lib/store.mjs: markPastudioImported/unmark/consume + pastudioNeedsModel.
 *
 ...
 * No auto-run/resume on import; imported roots require explicit model choice.
 */
...
export const PASTUDIO_COMPRESSED_MAX = 64 * 1024 * 1024;
export const PASTUDIO_TOKEN_TTL_MS = 15 * 60 * 1000;
export const PASTUDIO_MAX_CONCURRENT = 2;
...
export function createProjectArchives(options = {}) {
  const { store, roadmap, sessionDir, dataDir, agentHome } = options;
    ...
    for (const rec of records) {
      try {
        ...
        if (markerPresent) {
          ...
          // pending instead of a completed duplicate.
  ...
  async function decodeAndValidate(buffer) {
    ...
    if (buffer.length > PASTUDIO_COMPRESSED_MAX) throw fail(413, 'pastudio_too_large', 'Archive trop volumineuse (64 Mio maximum).');
  ...
  async function exportArchive(cwd) {
    ...
    try {
      ...
      catch (e) { throw fail(500, 'pastudio_encode_failed', 'Encodage impossible.'); }
  ...
  async function importArchive(buffer, destCwd, previewToken) {
    ...
    try {
      ...
      if (roadResult?.duplicate) {
        ...
        tokens.delete(previewToken);
        const counts = buildCounts(decoded.manifest, decoded.files, decoded.roadmapDoc);
        return { duplicate: true, archiveId: decoded.archiveId, payloadDigest: decoded.payloadDigest, destinationCwd: dest.cwd, imported: counts, idMap: {}, nextStep: 'choose-model', warnings: ['additive', 'no-overwrite', 'secret-in-history', 'needs-model-choice'] };
      }
      ...
      const warnings = ['additive', 'no-overwrite', 'secret-in-history', 'needs-model-choice'];
      ...
      if (marksOk && lineageOk) {
        ...
        return {
          ...
          nextStep: 'choose-model', warnings,
        };
      }
  ...
  // Model gate: imported sessions require an EXPLICIT available model per start.
  // B1: consults the store flag AND the pending journal — marks-pending ids
  // (mark absent after commit) stay gated until recovery replays the marks.
  // Pure gate decision (isolated truth-table checks): store flag OR pending
  // membership gates; total read failure is unknown (caller fails closed).
  ...
  async function checkPastudioModelGate(sessionId, explicitModelId, catalog) {
    // Fail CLOSED: if neither the store flag nor the pending journal can be
    // read, the session must not run unrestricted.
    ...
    if (!explicitModelId || typeof explicitModelId !== 'string' || !explicitModelId.trim())
      throw fail(409, 'pastudio_needs_model', 'Choisissez un modèle disponible pour démarrer cette conversation importée.');
    const found = catalog?.models?.find((m) => m.id === explicitModelId);
    if (!found) throw fail(400, 'pastudio_needs_model', 'Modèle choisi indisponible.');
    if (found.availability === 'unavailable') throw fail(409, 'pastudio_needs_model', 'Modèle choisi indisponible.');
    return { gated: true };
  }

  return { exportArchive, previewArchive, importArchive, recoverPending, checkPastudioModelGate, decidePastudioGate, computePayloadDigest, canonicalTopologyJson };
}
