...
*{box-sizing:border-box}body{margin:0;min-height:100dvh;background:#171c1e;color:#e8f0ee;font:16px/1.6 system-ui,sans-serif;display:grid;place-items:center;padding:24px}main{width:100%;max-width:410px}.brand{width:48px;height:48px;display:grid;place-items:center;background:#7ec9bc;color:#0f2a27;border-radius:14px;font-size:30px;font-weight:700;margin-bottom:36px}h1{font-size:34px;line-height:1.2;letter-spacing:-1px;font-weight:600;margin:0 0 16px}p{color:#8e9895;margin-bottom:30px}label{display:block;font-size:14px;margin:0 0 10px}input{display:block;width:100%;padding:16px;border:1px solid #3d4a48;border-radius:12px;background:#1e2427;color:#fff;font:24px system-ui;letter-spacing:6px;text-align:center}input:focus{outline:2px solid #7ec9bc;outline-offset:3px}button{width:100%;background:#7ec9bc;color:#0f2a27;border:0;border-radius:12px;padding:16px;font:600 15px system-ui;margin-top:18px;cursor:pointer}small{display:block;color:#8e9895;margin-top:22px;text-align:center;font-size:12px}.login-language{display:flex;align-items:center;justify-content:space-between;gap:16px;margin-top:24px}.login-language label{margin:0}.login-language select{max-width:60%;padding:10px;border:1px solid #3d4a48;border-radius:10px;background:#1e2427;color:#e8f0ee;font:14px system-ui}.error{color:#f0adad;font-size:14px;margin:12px 0}</style></head><body><main><div class="brand">P</div><h1 data-i18n="login.heading">${tr('login.heading', {}, locale)}</h1><p data-i18n="login.intro">${tr('login.intro', {}, locale)}</p><form method="post" action="/lan/login"><label for="code" data-i18n="login.code">${tr('login.code', {}, locale)}</label><input id="code" name="code" type="password" inputmode="numeric" autocomplete="one-time-code" pattern="[0-9 ]{8,12}" required maxlength="12" placeholder="••••••••" autofocus>${error ? `<div class="error" role="alert"${messageKey(error) ? ` data-i18n="${messageKey(error)}"` : ''}>${escapeHtml(knownMessage(error, locale))}</div>` : ''}<button type="submit" data-i18n="login.open">${tr('login.open', {}, locale)}</button></form><small data-i18n="${messageKey(accessLabel)}">${escapeHtml(knownMessage(accessLabel, locale))}</small><div class="login-language"><label for="login-language" data-i18n="language.label">${tr('language.label', {}, locale)}</label><select id="login-language" data-language-select></select></div></main></body></html>`;
