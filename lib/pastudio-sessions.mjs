 ...
 *   5. Resume gate (UI/backend, owned by parent): getResumeContract()
 *      declares requiresModelSelection:true. History keeps model_change
 *      records as evidence; the next start MUST take an explicit user-picked
 *      available model. This module never injects a default model.
 *
...
function checkHeaderFilename(header, file) {
  const base = String(file).split(/[\\/]/).pop().replace(/\.jsonl$/, '');
  if (header.id !== base)
    fail('pastudio_invalid', `Header id ${header.id} does not match filename ${file}; refusing an ambiguous native file.`, { file });
  return true;
}
...
export async function collectExport({
  ...
  for (const rootId of roots) {
    ...
    queue.push({ file, kind: 'root', parentOldSessionId: null, childId: null, depth: 0, edgeName: null });
  }
  while (queue.length) {
    ...
    const oldSessionId = parsed.header.id;
    if (oldSessionId !== item.file.split(/[\\/]/).pop().replace(/\.jsonl$/, ''))
      fail('id_filename_mismatch', `Header id ${oldSessionId} does not match filename ${item.file}; refusing to export an ambiguous native file.`, { file: item.file });
    if (visitedSessionIds.has(oldSessionId))
      fail('duplicate_session', `Session ${oldSessionId} reached twice in export selection; roots must be disjoint families.`, { sessionId: oldSessionId });
    visitedSessionIds.add(oldSessionId);
    if (item.kind === 'child') {
  ...
  const manifest = {
    ...
    roots: roots.map((id) => {
      const found = manifestSessions.find((s) => s.oldSessionId === id);
...
/**
 * Resume contract: history is evidence, the next start needs an explicit
 * user-picked available model. The backend/UI gate owns enforcement; this
 * module never injects or defaults a model.
 */
export function getResumeContract() {
  return {
    requiresModelSelection: true,
    modelPolicy: 'retain-history-require-explicit-pick',
    thinkingPolicy: 'retain-history-do-not-clamp',
    reason: 'Historical model_change/assistant.model records are evidence only; settings.json defaults are never imported. The user picks an available model before the first resumed start.',
  };
}
...
export async function collectProjectSessions({ store, sessionDir, agentHome, cwd, readEdges } = {}) {
  ...
  for (const file of rootFiles) {
    ...
    try {
      checkHeaderFilename(parsed.header, file);
    } catch (error) {
  ...
  while (queue.length) {
    ...
    for (const edge of siblings) {
      ...
      try {
        checkHeaderFilename(parsed.header, edge.child);
      } catch (error) {
