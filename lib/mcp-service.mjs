...
export function createMcpService({
  ...
  async function probePython() {
    // Prefer an explicit Prime Agent kernel override, then common PATH interpreters.
    // Studio no longer prepares a private uv kernel — Prime Agent bootstraps its own.
    ...
    const marked = localKernelPython(kernelRoot);
    if (marked && existsSync(marked)) return marked;
    for (const candidate of ['python3', 'python']) {
      try {
        const { spawnSync } = await import('node:child_process');
        const check = spawnSync(candidate, ['-c', 'print(1)'], {
          encoding: 'utf8',
          timeout: 5000,
          env: environment,
        });
