 ...
 *
 * Format: standard ZIP (stored method 0 for writes; stored/deflated for reads)
 * with whitelist:
 ...
 *
 * Writer (`encodePastudio`) emits deterministic stored (method 0) ZIP:
 * sorted names, flags 0x0800 (UTF-8), no extra, no comment, fixed DOS date.
 *
...
/**
 * Encode a `.pastudio` ZIP archive in memory (stored, deterministic).
 *
 ...
 * @returns {Buffer} ZIP file bytes (<= 64 MiB).
 ...
 * - All entries use method 0 (stored). No encryption, no extra, no comment.
 */
export function encodePastudio(entries) {
  ...
  for (let i = 0; i < normalized.length; i++) {
    localSize += 30 + nameBytesList[i].length + normalized[i].data.length;
    centralSize += 46 + nameBytesList[i].length;
  }
  const totalSize = localSize + centralSize + 22;
  if (totalSize > PASTUDIO_COMPRESSED_MAX)
    fail('pastudio_too_large', 'pastudio: encoded ZIP exceeds 64 MiB.');
 ...
 *
 * @param {Buffer|Uint8Array} buffer ZIP file bytes (<= 64 MiB).
...
export function decodePastudio(buffer) {
  ...
  if (buf.length > PASTUDIO_COMPRESSED_MAX)
    fail('pastudio_too_large', 'pastudio: ZIP exceeds 64 MiB.');
