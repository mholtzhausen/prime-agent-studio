...
export const cwdKey = (cwd) => resolve(cwd);
  ...
  return {
    ...
    async getStudioPreferences() {
      ...
      return {
        ...
        includeExtensionProviders: state.preferences?.includeExtensionProviders === true,
    ...
    async setStudioPreferences(patch) {
      ...
      if (
        keys.length !== 1 ||
        !['allowQuestionsByDefault', 'includeExtensionProviders'].includes(key) ||
    ...
    async unmarkPastudioImported(ids) {
      ...
      for (const sessionId of ids) {
        ...
        const { pastudioImported, pastudioNeedsModel, pastudioArchiveId, pastudioImportedAt, ...rest } =
          state.sessions[sessionId];
