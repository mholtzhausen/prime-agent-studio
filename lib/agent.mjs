...
import { resolvePrimeAgentNodeEntry } from '../runtime/npm-bridge.mjs';
...
/** Resolve npm's JavaScript entrypoint for the local Prime Agent package. */
export function discoverCli(explicit = process.env.PRIME_AGENT_CLI, env = process.env, { all = false } = {}) {
    ...
    try {
      if (statSync(path).isDirectory()) {
        try {
          ...
          // nvm/npm global bin dirs: accept …/bin or …/bin/ containing the prime-agent shim.
          const shim = join(path, 'prime-agent');
    ...
    for (let depth = 0; depth < 5; depth++, packageDir = dirname(packageDir)) {
      try {
        ...
        if (pkg.name === 'prime-agent')
          return { path, node: true, packageDir, version: cleanString(pkg.version) };
      } catch {
        /* Walk upward to the npm package root. */
      }
    }
    return { path, node: /\.(mjs|cjs|js)$/i.test(path), packageDir: null, version: null };
  }
  if (explicit) return inspect(explicit);
  const nodePrefix = dirname(dirname(process.execPath));
  const candidates = [
    join(ROOT, 'node_modules', 'prime-agent'),
    join(homedir(), '.local', 'lib', 'node_modules', 'prime-agent'),
    join(homedir(), '.local', 'share', 'prime-agent', 'node_modules', 'prime-agent'),
    join(homedir(), '.npm-global', 'lib', 'node_modules', 'prime-agent'),
    // nvm / official Node layouts: <prefix>/bin/node → <prefix>/lib/node_modules
    join(nodePrefix, 'lib', 'node_modules', 'prime-agent'),
  ...
  for (const dir of (env.PATH || '').split(delimiter)) {
    if (!dir) continue;
    candidates.push(join(dir, 'node_modules', 'prime-agent'), join(dir, 'prime-agent'));
  }
  // Well-known nvm installs when PATH lacks shims (common under desktop GUI sessions).
  try {
    const nvmRoot = join(homedir(), '.nvm/versions/node');
    if (existsSync(nvmRoot)) {
      for (const entry of readdirSync(nvmRoot)) {
        candidates.push(
          join(nvmRoot, entry, 'lib/node_modules/prime-agent'),
          join(nvmRoot, entry, 'bin/prime-agent'),
        );
      }
    }
  } catch {
...
export function agentEnvironment({ agentHome, sessionDir, env = process.env } = {}) {
  ...
  // AppImage AppRun injects PYTHONHOME/PYTHONPATH into a non-existent AppDir
  // Python tree; Studio does not ship Python, so drop those before agents run.
  if (result.APPDIR || result.APPIMAGE) {
    delete result.PYTHONHOME;
    delete result.PYTHON_HOME;
    delete result.PYTHONPATH;
    delete result.PYTHON_PATH;
  }
...
export function createAgentRuntime(options = {}) {
  ...
  let cli = discoverCli(options.cliPath);
  ...
  // Prime Agent bootstraps its own Python kernel; Studio does not inject a kernel hook.
  ...
  /** Refresh CLI / component env after System settings or desktop activation. */
  function reloadComponents(selected = {}) {
    for (const key of ['PRIME_AGENT_CLI', 'PRIME_STUDIO_COMPONENTS_REQUIRED']) {
      if (typeof selected[key] === 'string' && selected[key]) env[key] = selected[key];
      else if (selected[key] === null || selected[key] === '' || selected[key] === '0') {
        if (key === 'PRIME_STUDIO_COMPONENTS_REQUIRED') env[key] = selected[key] || '0';
        else delete env[key];
      }
    }
    if (selected.PRIME_STUDIO_COMPONENTS_REQUIRED === '0')
      process.env.PRIME_STUDIO_COMPONENTS_REQUIRED = '0';
    else if (selected.PRIME_STUDIO_COMPONENTS_REQUIRED === '1')
      process.env.PRIME_STUDIO_COMPONENTS_REQUIRED = '1';
    cli = discoverCli(env.PRIME_AGENT_CLI || options.cliPath || null, env);
    if (cli?.packageDir) {
      env.PRIME_STUDIO_SESSION_PACKAGE = cli.packageDir;
      env.PRIME_STUDIO_TRANSPORT_PACKAGE = cli.packageDir;
      if (options.subagentPolicyFile) env.PRIME_STUDIO_POLICY_CLI_ROOT = cli.packageDir;
    }
    statusPromise = undefined;
    return { cli: cli?.path || null, version: cli?.version || null };
  }
  ...
  function getStatus() {
    if (env.PRIME_STUDIO_COMPONENTS_REQUIRED === '1')
      return Promise.resolve({
        ...
        error:
          'Terminez la configuration dans Préférences → Système. / Finish setup in Preferences → System.',
      });
    if (!cli)
      return Promise.resolve({
        available: false,
  ...
  async function getModels({ refresh = false, includeExtensionProviders = false } = {}) {
    ...
    let nativeSnapshot;
    try {
      nativeSnapshot = await nativeModelCatalog?.read({
        refresh,
        includeExtensionProviders: includeExtensionProviders === true,
      });
    } catch {
