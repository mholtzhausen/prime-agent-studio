...
import { UPDATER_LATEST_JSON_URL, appImageDownloadUrlPattern } from './desktop-release.mjs';

// Fixed upstream only. Never accept a URL from any request body or file.
export const UPDATER_META_URL = UPDATER_LATEST_JSON_URL;
...
const SEMVER = /^(\d+)\.(\d+)\.(\d+)$/;
const INSTALLER_URL = appImageDownloadUrlPattern();
...
export function createRemoteUpdates({
  ...
  async function fetchPublishedFresh(force) {
    ...
    try {
      ...
      const target = data?.platforms?.['linux-x86_64'];
      if (
        !parseSemver(version) ||
        typeof notes !== 'string' ||
