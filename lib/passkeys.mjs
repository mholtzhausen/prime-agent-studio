...
export function createPasskeys({ dataDir }) {
  ...
  async function options({ kind, origin, epoch, binding, name }) {
    ...
    if (kind === 'register') {
      ...
      optionsJSON = await generateRegistrationOptions({ rpName: 'Prime Agent Studio Nix', rpID,
        userID: randomBytes(32), userName: 'Prime Agent Studio Nix', userDisplayName: 'Prime Agent Studio Nix',
